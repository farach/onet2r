...
* Cached archive and adapter readers now copy verified bytes to a private snapshot while holding the cache lock, and archive readers retain that exact acquired snapshot through parsing across forced refreshes. Omitted provenance fields remain unconstrained when a verified snapshot is reused. URL credential redaction now matches explicit case-insensitive, percent-decoded parameter names, including OAuth authorization `code` and cloud signatures, without hiding benign parameters such as `author`, `monkey`, or `state`.
