...
* `onet_cache_clear()` now coordinates with active cache transactions, accepts a configurable wait `timeout`, and uses ownership-checked lock cleanup, preventing concurrent clears and refreshes from deleting replacement locks or separating cached sources from their receipts.
...
* `onet_import_eloundou()` and `onet_import_felten_aioe()` now verify and parse private snapshots of local files, so mutations to the original path cannot separate parsed values from their receipt digest.
...
* Credential redaction now removes malformed multi-`@` user information through the final authority separator while preserving benign `@` characters in paths, queries, and fragments.
