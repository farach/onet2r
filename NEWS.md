...
* Cached archive and adapter readers now copy verified bytes to a private snapshot while holding the cache lock, so a concurrent refresh cannot separate parsed data from its source receipt. OAuth authorization `code` parameters are redacted with other URL credentials.
