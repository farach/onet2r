...
* `onet_content_change()` and `onet_task_to_occupation()` now reject duplicate effective keys instead of silently choosing or blending rows. Task rollups also require exactly one non-missing release per call when release metadata is present; pass named single-release tables to `onet_measure_sensitivity()` for multi-vintage analysis (reported in release audit).
