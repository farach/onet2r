...
* `onet_import_eloundou()` ingests the occupation-level GPT-exposure table from Eloundou et al. (2023), "GPTs are GPTs", as an occupation-grain `onet_measure()` object. It is a thin adapter that selects the key and score columns and records provenance without transforming the published values. The MIT-licensed data are never bundled; supply a local `path` or download `url`.
* `onet_import_felten_aioe()` ingests the AI Occupational Exposure (AIOE) scores from Felten, Raj, and Seamans (2021) as an occupation-grain `onet_measure()` object keyed on 6-digit SOC code, following the same download-only, no-transform adapter pattern.
* `onet_measure()` gains an `items = ` / `agg = ` convenience path: given a Task Ratings style panel, a set of target item ids, and `agg = "targeted"`, it builds an occupation-grain measure equal to each occupation's importance-weighted share of its item portfolio on the targeted items. The existing `data` / `key` / `score` / `key_type` path is unchanged.
